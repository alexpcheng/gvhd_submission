...
{
	dsystem ("cat $utr_fn | fasta2stab.pl > tmp_utr_stab_$r");
	dsystem ("cat $mir_fn | fasta2stab.pl > tmp_mir_stab_$r");
}
...
else
{
	...
	{
		dsystem ("cat $upstream_fn | fasta2stab.pl > tmp_upstream_stab_$r");
	}
	
	dsystem ("cat tmp_utr_stab_$r " .
			 "| cut -f 1 " .
			 "| add_column.pl -s $POLY_A " .
			 "| join.pl -o A - tmp_upstream_stab_$r " .
			 "| merge_columns.pl -1 1 -2 2 " .
			 "> tmp_ext_utr_$r;");
	
	dsystem ("cat tmp_ext_utr_$r " .
			 "| stab2length.pl " .
			 "| cut.pl -f 1,1,2,2 " .
			 "| modify_column.pl -c 2 -s " . ($CDS_FLANK-1) . " " .
			 ">tmp_ext_utr_pos_$r");
			 
	dsystem ("cat tmp_ext_utr_$r " .
			 "| extract_sequence.pl -f tmp_ext_utr_pos_$r " .
			 "> tmp_prefix_$r");
}

my $ext_utr_fn = $output_dir . $prefix . "ext_utr.stab";

dsystem ("join.pl tmp_prefix_$r tmp_utr_stab_$r " .
         "| add_column.pl -s $POLY_A " .
         "| merge_columns.pl -1 1 -2 2 " .
         "| merge_columns.pl -1 1 -2 2 " .
         "> " . $ext_utr_fn);
...
dsystem ("find_potential_mirna_targets.pl tmp_utr_stab_$r -f tmp_mir_stab_$r $echoed_pt_args > tmp_pt_$r");
...
dsystem ("cat tmp_pt_$r " .
		 "| join.pl -1 2 - $ext_utr_fn " .
		 "| cut.pl -f 1-9,11- " .
		 "| modify_column.pl -c 2,3 -a $CDS_FLANK " .
		 "| RNAddG_compute.pl -ddgarea $DDG_AREA -upstream_rest $FLANK_UP -downstream_rest $FLANK_DOWN " .
		 "| modify_column.pl -c 2,3 -s $CDS_FLANK " .
		 "| modify_column.pl -c 9,10,11,12,13,14 -m '\"-1\"' " .
		 "| cut.pl -f 1-15,14 " .
		 "| modify_column.pl -c 15 -sc 14 " .
		 "| cut.pl -f 1-12,14-16,13 " .
		 "| modify_column.pl -c 9,10,11,12,13,14,15 -p 2 -m '\"-1\"' " .
		 "| cap.pl \"UTR,microRNA,Start,End,Seed,Mismatchs,G:U,Loop,Site size,dGduplex,dG5,dG3,dG0,dG1,dGopen,ddG\" " .

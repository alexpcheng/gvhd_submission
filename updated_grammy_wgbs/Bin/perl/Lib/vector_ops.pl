...
#---------------------------------------------------------
# center (by reference) - actually normalize...
#---------------------------------------------------------
...
#---------------------------------------------------------
# pearson - note that the vectors are assumed to be normalized!!!
#---------------------------------------------------------
sub vec_pearson ($$)
{
  my ($vecX, $vecY) = @_;
...
#---------------------------------------------------------
# spearman - two vectors assumed to be of equal length
#---------------------------------------------------------
sub vec_spearman ($$)
{
  	my ($vecX, $vecY) = @_;
	
	my @temp_valsX = sort {$a <=> $b} @$vecX;
	my %temp_ranksX = {};
	my %countX = {};
	
	my @temp_valsY = sort {$a <=> $b} @$vecY;
	my %temp_ranksY = {};
	my %countY = {};
	
	for (my $i = 0; $i < @temp_valsX; $i++) 
	{
		if (length($temp_valsX[$i]) > 0) {
			if (exists($temp_ranksX{$temp_valsX[$i]})) {
				$temp_ranksX{$temp_valsX[$i]} += $i;
				$countX{$temp_valsX[$i]}++;
			}
			else {
				$temp_ranksX{$temp_valsX[$i]} = $i;
				$countX{$temp_valsX[$i]} = 1;
			}
		}
		if (length($temp_valsY[$i]) > 0) {
			if (exists($temp_ranksY{$temp_valsY[$i]})) {
				$temp_ranksY{$temp_valsY[$i]} += $i;
				$countY{$temp_valsY[$i]}++;
			}
			else {
				$temp_ranksY{$temp_valsY[$i]} = $i;
				$countY{$temp_valsY[$i]} = 1;
			}
		}
	}
	...
	for (my $i = 0; $i < @$vecX; $i++)
 	{
		$ranksX[$i] = (length($$vecX[$i]) > 0) ? $temp_ranksX{$$vecX[$i]}/$countX{$$vecX[$i]} : "";
		$ranksY[$i] = (length($$vecY[$i]) > 0) ? $temp_ranksY{$$vecY[$i]}/$countY{$$vecY[$i]} : "";
	}
...
#  	print "\n";
  	...
  	&vec_center_by_ref(\@ranksX);
  	&vec_center_by_ref(\@ranksY);
  	my $spearman = &vec_pearson(\@ranksX,\@ranksY);

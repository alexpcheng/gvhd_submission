...
my $box_width = get_arg("boxwidth", "", \%args);
my $fill_style = get_arg("fs", "", \%args);
my $point_size = get_arg("ps", 0.6, \%args);
my $line_width_all = get_arg("lwall", "", \%args);
...
my $smoothall = get_arg("smoothall", "", \%args);

# add the -ps option
if ($make_ps) { $make_postscript = 1; }

if ($x_label_column ne ""){ $x_label_column=":xtic(".$x_label_column.")" }
if ($y_label_column ne ""){ $y_label_column=":ytic(".$y_label_column.")" }
if ($make_postscript) { $output_format = "postscript eps enhanced color solid"; if ($font ne "") { $output_format .= " \"$font\" $fontsize";} }
...
{ 
   if ($font ne "")
   {
     ...
     print OUTFILE " font \"".$ENV{"GENIE_HOME"}."/Bin/fonts/$font\" $fontsize ";
   }
   else{
...
print OUTFILE "set style data $data_style\n";
if ($data_style eq "histograms"){
    print OUTFILE "set style histogram ".(($error_all_columns)?"errorbars":"cluster")." gap 1 lw 2\n";
}
if ($xrotate ne ""){
    print OUTFILE "set xtics rotate by $xrotate\n";
}

print OUTFILE "set pointsize $point_size\n";
if ($fill_style ne ""){
    print OUTFILE "set style fill $fill_style ".(($data_style eq "histograms")?" border -1":"")."\n";
}
...
if (length($xtics) > 0)
{ 
   if (length($xtics) == 0 and  $xtics == 0) { print OUTFILE "unset xtics\n"; }
   else { print OUTFILE "set xtics nomirror $xtics\n"; }
}
if (length($ytics) > 0) 
{ 
   if (length($ytics) == 0 and $ytics == 0) { print OUTFILE "unset ytics\n"; }
   else { print OUTFILE "set ytics nomirror $ytics\n"; }
}
...
if ($half_border_right) { print OUTFILE "set xtics nomirror\nset y2tics auto\nunset ytics\nset border 9\n"; }
...
elsif ($all_columns and $error_all_columns and $data_style eq "histograms"){
    $num_of_data_points = scalar(@x_columns)/2;
}
...
for (my $i = 0; $i < $num_of_data_points ; $i++)
{
  ...
  my $line_width = "";
  if (length($line_width_all) > 0) {$line_width = "lw $line_width_all"};
  if (length($line_widths[$i]) > 0) {$line_width = "lw $line_widths[$i]"};
  
  my $fill_style = length($fill_styles[$i]) > 0 ? "fs $fill_styles[$i]" : "";
  my $smooth = "";
  if (length($smoothes[$i]) > 0){
      $smooth = "smooth $smoothes[$i]";
  } 
  elsif (length($smoothall) > 0){
      $smooth = "smooth $smoothall";
  } 
  my $axis = length($axes[$i]) > 0 ? "axes $axes[$i]":"";

  my $temp_data_file = "tmp_".rand(int(10000000));

  system("body.pl ".(1+$skiprows)." -1 $data_file_name | tr \"\t\" \",\" | grep -v '^#' > $temp_data_file");
  ...
  else
  {
    ...
    if ($data_style eq "histograms"){
	print OUTFILE  "\"$temp_data_file\" using $y_columns[$i]$error_bar:xtic($x_columns[$i])$x_label_column$y_label_column $axis $plot_keys[$i] $key_data_style $line_type $point_type $point_size $line_width $fill_style $smooth";
    }
    else {
...
make_gnuplot_graph.pl <data file>
   ...
   -smoothall <str> when using -all you can use this flag to smooth all the columns in the same method
   ...
   -ds <style>:     Data style (boxes/line/point/linespoint/imp/histograms default: point)
   -boxwidth <num>: Set the box width (default: let gnuplot determine this automatically)
   -ps <num>:       Point size (default: 0.2)
   -fs <style>:     Fill style (solid/none default: none)
   ...
   -xrotate <num>    rotate the labels of the x-axis by <num> degrees
   ...
   -fontsize:       Fontsize for nonscalable font use tiny/small/medium/large/giant (default small).
                    For scalable font specify a pointsize (default 10).
   -font <str>:     Use a scalable ttf/pfa font (choose from /home/genie/Genie/Bin/fonts/ , default Tahoma.ttf when -font is used)

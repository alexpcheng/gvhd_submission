...
my $no_print = get_arg("no_print", 0, \%args);
...
sub order_rows
{
  ...
  while(<SOURCE_FILE>)
  {
    ...
    if (length($ordered_row) > 0 and $no_print == 0) { print OUTFILE "$ordered_row\n"; }
  }
...
order_file.pl <source file> <order file>
   ...
   -no_print:     Do *NOT* print the rows/columns found in source file

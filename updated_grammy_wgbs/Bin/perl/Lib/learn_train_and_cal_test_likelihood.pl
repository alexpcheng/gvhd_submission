...
----------------------------- 1. running cv -------------------------------------
learn_train_and_cal_test_likelihood.pl <dirs_file> <run_out_file_name> <cv_groups_num> 
                                              <background_matrix_file> <train_positive_file_name_prefix> 
	...
	-b <background_file_name_prefix> default is background if NON then no backgrouns file
	-l <letters_at_position_feature_max_positions_num>
	-c <sum_weights_penalty_coefficient>
	-w <num_of_sequences_in_the_world>
	-f <positive_seqs_prior_probability>
	-q <queue max length> for not creating to busy queue
	-e <estimate_measure_type> default is Default (can also be AllInLasso)
	-if <use_initial_p_value_filter> default is true , boolean, for initial filter
	-ipf <filter_only_positive_features> default is true , boolean, for initial filter
	-r <remove_features_under_weight_thresh> default is 0.001 for allin laso learning
	-UseOnlyPositiveWeights <use_only_positive_weights: true/false> default is true 
	-mypssm <0/1> default is 0 , if 1 learns also mypssm
	-math <cal_weights_functions_samples> cal matlab style sample from functions default is false 
	-reweight <reweight_positive_instances_fraction> true/false, default is true
	-percentFilterThresh <initial_filter_count_percent_thresh> defualt 0.03 the thresh of the initial percent form positive sequences
	-writeIterationFile <0/1> default is 0 , if 1 write iteration file 
	-majorIter <0/1> major_training_parameter_max_train_iterations default is 1000
	-secondaryIter <0/1> secondary_training_parameter_max_train_iterations default is 1000
	-useSecondaryTrain <use_secondary_training_procedure> true/false default is true
	-onlyCVNum <run_only_on_single_cv_num> #, defualt 0 
	-runMode <run_mode>  can be: "FullRun", "OnlyCreateCV", "OnlyRunOnSingleCV" ,  defualt "FullRun", "OnlyCollectRes"
	-dontCreateCVDirs <0/1> defualt is zero, if one no dirs will be created
	-pssmImportanceSampling <0/1> defualt is zero, if one uses pssm importance sampling
	-majorTrainingProcedureType <ConjugateGradient (defualt) / Simplex / ConjugateGradientUsingOnlyGradient > the method for convex optimization in the major train

	-FunctionType <SeqsLGradientUsingFeatureEstimation/SeqsLUsingZFunc/SeqsLFunctionUsingExactZAndFeatureEstimation> defualt is SeqsLGradientUsingFeatureEstimation
	-FeatureSelection <Grafting/GainBased> defualt is Grafting
	-ExpectationEstimation <CliqueGraph/Sampling> defualt is CliqueGraph
	-EBySeqReweight <true/false> defualt is false
	-EBySeqOnlyZero <true/false> defualt is true
	-EBySeqImportance <true/false> defualt is true

	-LoopySpanTreeReduction <true/false> defualt is true
	-LoopyUseGBP <true/false> defualt is true
	-LoopyUseOnlyExact <true/false> defualt is false
	-LoopyMaxIter <uint> defualt is 1000
	-LoopyCalibrationT <[0,1]> defualt is 0.0001
	-LoopyCalibrationNodeP <[0,1]> defualt is 1
	-LoopyPotential <CpdPotential/LettersAtPositionsPotential> defualt is CpdPotential
	-LoopyDistance <DmNormLInf/DmNormL1/DmKlDistance> defualt is DmNormLInf
	-LoopyCalibrationTSuccess <[0,1]> defualt is 0.02
	-LoopyCalibrationNodePSuccess <[0,1]> defualt is 0.95
	-LoopyCalibrationMethod <[SynchronicBP,AsynchronyRBP]> defualt is SynchronicBP
	-LoopyCalibrationAverageMessagesInMessageUpdate <[0/1]> defualt is 0

	-init_fmm_from_pssm <[true,false]> defualt is false

	-limit_the_num_of_parameters_ratio_to_pssm <[#]> defualt is 3
	-max_learning_iterations_num <[#]> defualt is 100

	-structure_learning_sum_weights_penalty_coefficient <true/false> defualt is false
	-parameters_learning_sum_weights_penalty_coefficient <true/false> defualt is false
	-feature_selection_score_thresh <[#]> defualt is 0

	-remove_features_under_weight_thresh_after_each_iter <[true,false]> defualt is false
	-remove_features_under_weight_thresh_after_full_grafting <[true,false]> defualt is false
	-do_parameters_learning_iteration_after_learning_structure <[true,false]> defualt is false
	
	-pssm_pseudo_counts <double> defualt is 0.4
	-pseudo_count_equivalent_size <double> defualt is 5

	-l_at_two_pos_chi2_filter_fdr_thresh <double> defualt is -1 , under zero=not active
	-l_at_multiple_pos_binomial_filter_fdr <double> defualt is -1 , under zero=not active
	-multiple_hypothesis_correction <method> defualt is FDR
	
	-compute_partition_function_method <method: ComputeZFuncUsingCliqueGraph / ComputeZFuncOverLabeledSeqs> defualt is ComputeZFuncUsingCliqueGraph

	-realign_sequences <[true,false]> defualt is false
	-max_realign_iterations_num <[#]> defualt is 0

	-compute_positive_sequences_likelihood_method_type <[SumOverSequencePositions,BestPositionLikelihood]> defualt is SumOverSequencePositions

		-forced_num_positions_without_pedding_of_aligning_pssm <[#]> defualt is 0 if zero then no realign, if positive = num of position to decrease from len

----------------------------- 2. collecting cv results --------------------------

learn_train_and_cal_test_likelihood.pl --collect <dirs_file> <run_out_file_name> <common_out_file_path> <not run out file>


the second param is for each run out file, 
the third for the common output file 
the fourth is the list of not run for rerun

----------------------------- 3. collecting cv results --------------------------

learn_train_and_cal_test_likelihood.pl --ConcatFeaturesStatOutFiles <dirs_file> <run_out_file_name> <common_out_file_path> <num_of_cv_groups>


the second param is for each run out file, 
the third for the common output file of features stat
the fourth is the  num_of_cv_groups

------------------------------ 4. calculate sequences features indications -------

learn_train_and_cal_test_likelihood.pl --Indications <dirs_file> <CV_dir_num_for_model> <out_file_name>
                                                     <input_files_prefix> <processes_num>

<dirs_file>                            - the tabular dirs file as in the run = dir appearences length  
<CV_dir_num_for_model>                 - # of CV dir to use (CV_#)
<out_file_name>                        - outfile name
<input_files_prefix> <processes_num>   - input file name (in MacIsaac run - positive)

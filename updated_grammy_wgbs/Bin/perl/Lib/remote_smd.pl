...
foreach my $archive (@archives)
{
   ...
   my $dir_name = &getPathPrefix($archive);

   print STDERR "archive='$archive' --> base_name='$base_name' dir='$dir_name'\n";

   if($archive =~ /tar.gz$/)
   {
      print STDERR "Unzipping and untarring '$base_name'.\n";
      system("cd $remote_dir; gunzip -c ../$dir_name/$base_name | tar -xf -");
      print STDERR "Done unzipping and untarring.\n";
   }
   elsif($archive =~ /.tar$/)
   {
      print STDERR "Untarring '$base_name'.\n";
      system("cd $remote_dir; tar -xf $base_name");

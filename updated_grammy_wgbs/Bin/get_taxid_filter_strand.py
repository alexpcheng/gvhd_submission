...
with open(outfile, 'w') as w:
    for line in sys.stdin:
        ...
        if conversion == 'CT' and strand == 'Plus/Plus':
            if ('-1' in qseqid or '-C' in qseqid):
                w.write('\t'.join([qseqid, sseqid, pident, length, mismatch, gapopen, qstart, qend, sstart, send, evalue, bitscore, qlen, strand, taxid]) + '\n')
            if '-2' in qseqid: # we flip the coordinated on sstart and ssed because we did a reverse complement on R2s
                w.write('\t'.join([qseqid, sseqid, pident, length, mismatch, gapopen, qstart, qend, send, sstart, evalue, bitscore, qlen, strand, taxid]) + '\n')

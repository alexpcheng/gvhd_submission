...
skin <- df2[, c("patient_id", "skin", "total_cfDNA", "cluster_id",
                "days_post_HCT", "event", "gvhd", "days_HCT_to_GVHD_1",
                "skin_stage", "stauts")]

skin <- df2
skin <- skin[skin$days_post_HCT>0, ]
skin <- skin[is.na(skin$skin_stage) | skin$skin_stage!=0, ]

skin2 <- skin[skin$event!="Engraftment", ]
skin2$ss <- skin2$skin*skin2$total_cfDNA
wilcox.test(skin2$ss~skin2$stauts)
aggregate(.~stauts, skin2[, c('stauts', 'ss')], mean)
table(skin2$stauts)
pdf('figures/SKIN_GVHD-v1_conc.pdf', width = 45/25.54, height = 90/25.54, useDingbats = FALSE)
ggplot(data =skin2, aes(x=stauts, y=ss), fill = "white")+
  stat_summary(geom = "bar", fun.y = "mean", fill = "red", color = "black")+
  stat_summary(geom="errorbar", width = 0.2)+
  theme_classic()+
  ylab("Skin-derived cfDNA")+
  xlab("GVHD status")+
...
skin2$ss <- skin2$skin*skin2$total_cfDNA
skin2$gr <- NA
skin2$gr[skin2$stauts=="-"]<- "1No GVHD"
skin2$gr[grepl("0", skin2$skin_stage)]<-"GVHD+/skin-"
skin2$gr[grepl("1|2", skin2$skin_stage)]<-"skin1/2"
skin2$gr[grepl("3|4", skin2$skin_stage)]<-"skin3/4"
table(skin2$gr)
skin3 <- skin2[grepl("1No", skin2$gr) | grepl("3/4", skin2$gr), ]

wilcox.test(skin3$ss~skin3$gr)
aggregate(.~gr, skin3[, c('gr', 'ss')], mean)


pdf('figures/SKIN_GVHD-v2_conc.pdf', width = 89/25.54, height = 90/25.54, useDingbats = FALSE)
ggplot(data =skin2, aes(x=gr, y=ss*total_cfDNA), fill = "white")+
  stat_summary(geom = "bar", fun.y = "mean", fill = "darkred", color = "black")+
  stat_summary(geom="errorbar", width = 0.2)+
  theme_classic()+
  ylab("Skin-derived cfDNA")+
  xlab("GVHD status")+
...
pdf('figures/SKIN_GVHD-v2_frac.pdf', width = 89/25.54, height = 90/25.54, useDingbats = FALSE)
ggplot(data =skin2, aes(x=gr, y=ss), fill = "white")+
  stat_summary(geom = "bar", fun.y = "mean", fill = "lightblue", color = "black")+
  stat_summary(geom="errorbar", width = 0.2)+
  theme_classic()+
  ylab("Skin cfDNA fraction")+
  xlab("GVHD status")+
  theme(axis.text = element_text(family = "Helvetica", size=6),
        axis.title = element_text(family = "Helvetica", size = 8))
dev.off()
